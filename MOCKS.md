...
| 🟡 Mock | Notificaciones (email/SMS) | Con `STRIPE_DEMO_MODE=true`, las notificaciones se guardan en la DB con status `SENT` pero no se envía ningún email ni SMS real. Se loguea en consola. | `STRIPE_DEMO_MODE=true` | Configurar proveedor de email (ej: SendGrid, SES) y SMS (ej: Twilio), setear `STRIPE_DEMO_MODE=false` | — |
