    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0a0a0a] px-4">
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          <h1 className="text-xl font-semibold text-white tracking-tight">Gestor de Citas Oficiales</h1>
          <p className="text-sm text-[#737373] mt-1">Iniciá sesión en tu cuenta</p>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <input id="password" type="password" autoComplete="current-password"
                {...register('password')} className={inputClass} />
              {errors.password && <p className="text-red-400 text-xs mt-1">{errors.password.message}</p>}
            </div>

            {needsMfa && (
              <div>
                <label htmlFor="mfaToken" className="block text-xs font-medium text-[#a3a3a3] mb-1.5">Código MFA</label>
                <input id="mfaToken" type="text" inputMode="numeric" maxLength={6}
                  {...register('mfaToken')} className={inputClass} />
              </div>
            )}
        ...
        <p className="mt-5 text-center text-sm text-[#525252]">
          ¿No tenés cuenta?{' '}
          <Link href="/register" className="text-white hover:underline">Registrate</Link>
        </p>
