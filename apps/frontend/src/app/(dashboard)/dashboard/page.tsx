    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-lg border border-border">
        <div className="p-4 border-b border-border flex items-center justify-between">
          <h3 className="font-medium text-foreground">Expedientes recientes</h3>
          <Link href="/bookings">
        ...
        {bookings.length === 0 ? (
          ...
          <div className="divide-y divide-border">
            {bookings.slice(0, 5).map((booking: any) => {
              const status = statusLabels[booking.status] || { label: booking.status, color: 'bg-secondary text-muted-foreground' };
              return (
                <Link key={booking.id} href={`/bookings/${booking.id}`} className="flex items-center justify-between p-4 hover:bg-input transition-colors">
                  <div>
                    <p className="text-sm font-medium text-foreground">{booking.procedure?.name}</p>
      ...
      <div className="bg-card rounded-lg border border-border p-4">
        <h3 className="font-medium mb-3 text-foreground">Acciones rápidas</h3>
